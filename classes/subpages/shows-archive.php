...
class ShowsArchive extends Subpage
{
	/**
	 * The subpage id
	 *
	 ...
	 * @since 1.0.0
	 */
	protected static $type = 'shows-archive';
	...
	public function getContent()
	{
		...
		$this->DB->query("SELECT task_gid,name,completed,custom_fields,due_on,resource_subtype,start_on FROM task WHERE task_gid IN(" . implode(",", $this->project['tasks']) . ");");
		...
		if ( is_array($this->project['tasks']) && count($this->project['tasks']) > 0 )
		{
			foreach( $this->project['tasks'] as $r )
			{
				if ( isset($this->tasks[$r]) && is_array($this->tasks[$r]) )
				{
					...
					{
						$out .= $this->display->compiledTemplates('skin_agenda')->showArchive( $this->tasks[$r] );
					}
				}
			}
		}
...
class ShowsArchiveType extends SubpageType
{
	/**
